...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Convert markdown explanation back to stylized HTML structures
    ...
    if (explainBtn) {
        explainBtn.addEventListener('click', async () => {
            ...
            try {
                utils.showLoader();
                reportContainer.innerHTML = '<div style="color: var(--text-muted);">// Explaining code... Please wait...</div>';
                ...
                utils.showToast('Code explanation completed!', 'success');
            } catch (err) {
                reportContainer.innerHTML = `<div style="color: #ff4757;">Error completing explanation: ${err.message}</div>`;
                utils.showToast(err.message || 'Error occurred during explanation', 'error');
            } finally {
                utils.hideLoader();
            }
        });
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            if (!fullRawReport) {
                utils.showToast('No explanation report available to copy', 'error');
                return;
            }
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            if (!fullRawReport) {
                utils.showToast('No explanation report available to download', 'error');
                return;
            }
            utils.downloadFile('code_explanation_report.md', fullRawReport);
        });
    }
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            ...
            reportContainer.innerHTML = '<div style="color: var(--text-muted);">// Paste code and click Explain Code to view detailed analysis...</div>';
            fullRawReport = '';
